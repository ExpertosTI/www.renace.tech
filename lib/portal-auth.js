...
async function issueSsoRedirect(pool, { portalUserId, instanceId, odooLogin, odooUrl, publicUrl, sessionId, portalBaseUrl, ip, userAgent }) {
  ...
  const portalBase = String(portalBaseUrl || process.env.NEXT_PUBLIC_BASE_URL || 'https://renace.tech').replace(/\/$/, '');
  ...
  await pool.query(
    ...
    [ssoToken, portalUserId, instanceId, odooLogin, expiresAt, ip || null, userAgent || '', sessionId || null, publicBase]
  );
  return {
    ssoToken,
    ssoRedirectUrl: `${portalBase}/api/sso/enter?token=${ssoToken}`,
    expiresAt: expiresAt.toISOString(),
  };
}
