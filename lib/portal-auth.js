...
/** Default public base ONLY for the main RENACE instance when no per-client public_url is set. */
 ...
 * Prefer per-instance publicUrl (each client has their own domain).
 * Never send every tenant to app.renace.tech — that is only the main-system fallback.
 */
function toPublicOdooUrl(odooUrl, publicUrl) {
  ...
  // Internal/IP without a public_url: main-system fallback only
  return ODOO_PUBLIC_BASE_URL;
}
