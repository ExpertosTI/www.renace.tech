...
async function issueSsoRedirect(pool, { portalUserId, instanceId, odooLogin, odooUrl, ip, userAgent }) {
  if (!pool || !portalUserId || !instanceId || !odooLogin || !odooUrl) {
    return { ssoRedirectUrl: `${String(odooUrl || '').replace(/\/$/, '')}/web/login` };
  }
  ...
  return {
    ssoToken,
    ssoRedirectUrl: `${String(odooUrl).replace(/\/$/, '')}/renace/sso?token=${ssoToken}`,
    expiresAt: expiresAt.toISOString(),
  };
}
