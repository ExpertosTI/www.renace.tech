(() => {
  ...
  const WEB_METRICS_API = 'https://ai.renace.tech/webhook/6a27d02f-7cee-49ba-bf7a-0b62fb136b60';
  ...
  function calculateInsights(data) {
    ...
    const sectorCount = {};
    submissions.forEach(s => {
      const key = String(s.sector || 'general');
  ...
  async function loadAnalytics() {
    ...
    try {
      ...
      // Load Web Analytics (Async, don't block main render)
      loadWebAnalytics();
  ...
  async function loadWebAnalytics() {
    try {
      const response = await fetch(WEB_METRICS_API);
      ...
      const getNum = (key) => {
          const val = data[key];
          if (Array.isArray(val) && val.length > 0) return Number(val[0][key] || 0);
          return Number(val || 0);
      };
      ...
      // KPIs
      document.getElementById('web-users-7d').textContent = getNum('usuarios_7d');
      document.getElementById('web-users-24h').textContent = `${getNum('usuarios_24h')} en 24h`;
      
      document.getElementById('web-events-7d').textContent = getNum('eventos_7d');
      document.getElementById('web-events-24h').textContent = `${getNum('eventos_24h')} en 24h`;

      document.getElementById('web-clicks-7d').textContent = getNum('clics_7d');
      document.getElementById('web-clicks-24h').textContent = `${getNum('clics_24h')} en 24h`;

      // Tables
      const referrers = safeParse(data.top_referrers);
      const pages = safeParse(data.top_paginas);

      const refList = document.getElementById('web-referrers');
      if (refList) {
        refList.innerHTML = referrers.length 
          ? referrers.map(r => `<li><strong>${escapeHtml(r.referrer || 'Directo')}</strong><span class="muted">${r.visits}</span></li>`).join('')
          : '<li><span class="muted">Sin datos de origen</span></li>';
      }

      const pageList = document.getElementById('web-pages');
      if (pageList) {
        pageList.innerHTML = pages.length
          ? pages.map(p => `<li><strong>${escapeHtml(p.path)}</strong><span class="muted">${p.views}</span></li>`).join('')
          : '<li><span class="muted">Sin datos de páginas</span></li>';
      }
    } catch (e) {
      console.warn('Web analytics failed:', e);
    }
  }
  ...
  function renderAnalytics(data) {
    ...
    document.getElementById('visits-24h-stat').textContent = visits.last24h ?? '-';
    document.getElementById('visits-source-label').textContent = `Fuente: ${formatVisitSource(visits.source)}`;
    ...
    const salesAvg = sales.count > 0 ? sales.totalAmount / sales.count : 0;
    document.getElementById('sales-avg').textContent = salesAvg ? formatMoney(salesAvg) : '-';
    ...
    document.getElementById('submissions-24h').innerHTML = insights.submissions24h > 0 
      ? `<span style="color:var(--good); font-weight:700;">+${insights.submissions24h}</span> nuevas hoy` 
      : 'Sin solicitudes hoy';
    ...
    const pipelineBar = document.getElementById('pipeline-health-bar');
    if (pipelineBar) pipelineBar.style.width = `${insights.pipelineHealth}%`;
    document.getElementById('forecast-value').textContent = formatMoney(insights.forecast);

    const topS = insights.topSectors[0];
    document.getElementById('top-sector').textContent = topS ? getSectorLabel(topS[0]) : '-';
    document.getElementById('top-sector-desc').textContent = topS ? `${topS[1]} solicitudes` : '-';

    const topM = insights.topModules[0];
    document.getElementById('top-module').textContent = topM ? topM[0].replaceAll('_', ' ') : '-';
    document.getElementById('top-module-desc').textContent = topM ? `${topM[1]} menciones` : '-';
  ...
  async function loadInstances() {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { instancesTbody.innerHTML = `<tr><td colspan="7" class="error">${escapeHtml(data.error)}</td></tr>`; return; }
      if (!data.length) { instancesTbody.innerHTML = '<tr><td colspan="7" class="muted" style="padding:14px 8px;">Sin instancias registradas.</td></tr>'; return; }
      instancesTbody.innerHTML = data.map(inst => `
        <tr>
          ...
          <td style="white-space:nowrap;">
            <button class="icon-btn-sm" onclick="window.toggleInstance(${inst.id},${!inst.active},'${escapeHtml(inst.client_name)}','${escapeHtml(inst.odoo_url)}','${escapeHtml(inst.public_url || '')}','${escapeHtml(inst.odoo_db)}','${escapeHtml(inst.service_code || '')}')">${inst.active ? 'Desactivar' : 'Activar'}</button>
            <button class="icon-btn-sm danger" onclick="window.deleteInstance(${inst.id})">Eliminar</button>
          </td>
        </tr>`).join('');
      populateInstanceSelect(data);
    } catch (e) { if (instancesTbody) instancesTbody.innerHTML = `<tr><td colspan="7" class="error">${escapeHtml(e.message)}</td></tr>`; }
  }
  ...
  window.toggleInstance = async function(id, active, client_name, odoo_url, public_url, odoo_db, service_code) {
    try {
      const res = await adminFetch(`/api/admin/odoo-instances/${id}`, { method: 'PUT', body: JSON.stringify({ client_name, odoo_url, public_url, odoo_db, service_code, active }) });
      const data = await res.json();
  ...
  document.getElementById('btn-add-instance')?.addEventListener('click', async () => {
    ...
    const public_url   = document.getElementById('inst-public-url')?.value.trim();
    ...
    try {
      const res = await adminFetch('/api/admin/odoo-instances', { method: 'POST', body: JSON.stringify({ service_code, client_name, odoo_url, public_url, odoo_db }) });
      const data = await res.json();
      if (!res.ok) { odooMsg(instancesMsg, data.error, 'error'); return; }
